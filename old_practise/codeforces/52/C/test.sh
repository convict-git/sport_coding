...
if ! g++ -g -std=gnu++14 -Wall -Wextra -Wshadow -Wfloat-equal -Wconversion -Wshift-overflow=2 -Wduplicated-cond -D_FORTIFY_SOURCE=2 -fsanitize=signed-integer-overflow -fsanitize=bounds -O2 $DBG C.cpp; then
    exit
fi
INPUT_NAME=input
OUTPUT_NAME=output
