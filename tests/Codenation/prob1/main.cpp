...
void solve() {
   ...
   for (int i = 0; i < n; ++i) {
      ...
      int k = sz - pi[sz - 1];
      if (sz % k == 0) {
         hashes.emplace_back(ha(0, k, mx), sz);
      }
      else hashes.emplace_back(ha(0, sz, mx), sz);
   }

   sort (hashes.begin(), hashes.end());

   int table_sz = (int)hashes.size();
   for (int i = 0; i < table_sz; ++i) {
      cerr << i << ' ' <<  hashes[i].x.x << ' ' << hashes[i].x.y << ' ' << hashes[i].y << '\n';
   }
   cin >> q;
   for (int i = 0; i < q; ++i) {
      string s;
      ...
      cerr << "TO MAT : " << to_mat.x << ' ' << to_mat.y << '\n';

      int l = 0, h = table_sz - 1;
      while (l <= h) {
         int g = (l + h)/2;
         if (hashes[g].x <= to_mat) {
            h = g - 1;
         }
         else l = g + 1;
      }

      if (l == table_sz || h == -1) cout << "NO\n";
      else {
         bool ok = false;
         for (int j = l; hashes[j].x == to_mat; ++j) {
            if (sz % hashes[j].y == 0) {
               ok = true;
               cout << "YES\n";
            }
