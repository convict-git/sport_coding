...
signed main() {
  ...
  for (int T = 1; T <= tc; ++T) {
    ...
    cin >> W >> H >> L >> U >> R >> D;
    --L, --U, --R, --D;
    vector <vector <ff>> m(H, vector <ff>(W));
    m[0][0] = 1.0;

#define met(r, c) ((long double) 1)/((r + 1 < H) + (c + 1 < W))

    for (int r = 0; r < H; ++r) {
      for (int c = 0; c < W; ++c) {
        if (r + 1 < H) {
          m[r + 1][c] += m[r][c] * met(r, c);
        }
        if (c + 1 < W) {
          m[r][c + 1] += m[r][c] * met(r, c);
        }
      }
    }
    ...
    for (int c = L; c <= R; ++c) {
      if (U - 1 >= 0) {
        res += m[U - 1][c] * met(U - 1, c);
      }
    }
    for (int r = U; r <= D; ++r) {
      if (L - 1 >= 0) {
        res += m[r][L - 1] * met(r, L - 1);
      }
    }
    cout << 1.0 - res << '\n';
  }
