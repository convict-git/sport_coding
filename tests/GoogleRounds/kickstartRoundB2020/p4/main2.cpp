...
inline ff CL(int n, int r) {
  return (facl[n] - facl[n - r] - facl[r]);
}
...
signed main() {
  ...
  for (int T = 1; T <= tc; ++T) {
    ...
    if (U - 1 >= 0) {
      for (int c = L; c <= R; ++c) {
        ff p = CL(U - 1 + c, c) + CL(H - U + W - c, H - U) - CL(H + W, H);
        p = exp(p);
        res += p;
      }
    }
    if (L - 1>= 0) {
      for (int r = U; r <= D; ++r){
        ff p = CL(L - 1 + r, r) + CL(H - r + W - L, H - r) - CL(H + W, H);
        p = exp(p);
        res += p;
      }
    }
