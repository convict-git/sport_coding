...
pair <ll, ll> solve(int x = 0, int y = n - 1) {
  ll w = 0, h = 0;
  for (int i = x; i <= y; ++i) {
    if (s[i] == 'N') {
      --h;
      ...
      pair <ll, ll> gt = solve(st, i - 1);
      w += (1ll *gt.first * cnt) % Mod, h += (1ll * gt.second * cnt) % Mod;
      w %= Mod, h %= Mod;
    }
  }
...
signed main() {
  ...
  for (int T = 1; T <= tc; ++T) {
    ...
    n = (int)s.size();
    pair <ll, ll> gt = solve();
    cout << ((gt.first + Mod) % Mod) + 1 << ' ' << ((gt.second + Mod) % Mod) + 1 << "\n";
  }
