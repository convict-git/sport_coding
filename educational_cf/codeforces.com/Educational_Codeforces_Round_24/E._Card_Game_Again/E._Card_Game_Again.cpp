...
vector <pair <int, int>> prime_factors(int k) {
  ...
  for (int p : primes) {
    ...
    if (alpha != 0) dvs.push_back(make_pair(p, alpha));
  }
  if (k != 1) dvs.push_back(make_pair(k, 1));
  return dvs;
}
...
signed main() {
  ...
  vector <pair <int, int>> divisors = prime_factors (k);
  int tp = (int)divisors.size();
  vector <vector <int>> prefix(n, vector <int> (tp));

  auto update = [&] (int i, int x) {
    vector <int> cur(tp);
    for (int j = 0; j < tp; ++j) {
      int p = divisors[j].first;
      int alpha = 0;
      while (x % p == 0) {
        ++alpha;
        x /= p;
    ...
    for (int j = 0; j < tp; ++j) {
      prefix[i][j] = cur[j] + (i ? prefix[i-1][j] : 0);
    }
  };

  auto is_divisible = [&] (int l, int r) -> bool {
    for (int j = 0; j < tp; ++j) {
      int cum_j = prefix[r][j] - (l > 0 ? prefix[l - 1][j] : 0);
      if (cum_j < divisors[j].second) return false;
    }
    return true;
  };
