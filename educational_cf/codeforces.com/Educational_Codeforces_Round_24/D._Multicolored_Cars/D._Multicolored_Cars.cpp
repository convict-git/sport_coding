...
signed main() {
  ...
  set <pair <int, int>> s; // (pos, Color) -> remove color with pos < posAlice
  vector <int> curpos(N);
  vector <int> eligible(N, 1);

  for (int Ci = 1; Ci <= N - 10; ++Ci) {
    if (Ci != Ca) s.insert(make_pair(0, Ci));
    curpos[Ci] = 0;
  }

  for (int i = 0; i < n; ++i) {
    int Ci = ar[i];
    int lastpos = curpos[Ci];
    ++curpos[Ci];

    if (Ci != Ca && eligible[Ci]) {
      s.erase(make_pair(lastpos, Ci));
      s.insert(make_pair(lastpos + 1, Ci));
    }

    while (!s.empty() && s.begin()->first < curpos[Ca]) {
      Ci = s.begin()->second;
      s.erase(s.begin());
      eligible[Ci] = false;
    }
