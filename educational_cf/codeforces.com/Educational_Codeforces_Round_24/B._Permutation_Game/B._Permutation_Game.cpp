...
signed main() {
  ...
  bool has = true;
  for (int i = 0; i < m - 1; ++i) {
    int df, cur_leader = l[i], next_leader = l[i + 1];
    if (cur_leader >= next_leader) {
      df = n - (cur_leader - next_leader);
    }
    else {
      df = (next_leader - cur_leader);
    }

    if (ar[cur_leader] == df) continue;
    else if (ar[cur_leader] || taken[df]) {
      has = false;
      break;
    }
    ar[cur_leader] = df;
