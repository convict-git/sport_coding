...
signed main() {
  ...
  for (int i = 0; i < n; ++i) {
    int u, v;
    cin >> u >> v;
    ar[i] = make_pair(min(u, v), max(u, v));
  ...
  for (int i = 0; i < n; ++i) {
    for (int j = i + 1; j < n; ++j) {
      // horizonatally
      int h = max(ar[i].first, ar[j].first);
      if (h <= a && ar[i].second + ar[j].second <= b) {
        res = max(ar[i].first * ar[i].second + ar[j].first * ar[j].second, res);
      }
      if (h <= b && ar[i].second + ar[j].second <= a) {
        res = max(ar[i].first * ar[i].second + ar[j].first * ar[j].second, res);
      }
      int v = max(ar[i].second, ar[j].second);
      if (v <= a && ar[i].first + ar[j].first <= b) {
        res = max(ar[i].first * ar[i].second + ar[j].first * ar[j].second, res);
      }
      if (v <= b && ar[i].first + ar[j].first <= a) {
        res = max(ar[i].first * ar[i].second + ar[j].first * ar[j].second, res);
      }
    }
  }
