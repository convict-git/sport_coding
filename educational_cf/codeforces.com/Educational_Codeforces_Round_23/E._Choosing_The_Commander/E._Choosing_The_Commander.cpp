...
inline int node (int cur, int bit) {
    if (pi[cur][bit] == -1) pi[cur][bit] = ++idx;
    return pi[cur][bit];
}

void modify (int x, int fc) {
    int cur = 0; f[cur] += fc;
    for (int d = 30; d >= 0; --d) {
        cur = node(cur, (x >> d) & 1);
        f[cur] += fc;
...
int calc (int xx, int yy) {
    int res = 0, cur = 0;
    for (int d = 30; d >= 0; --d) {
        int x = (xx >> d) & 1, y = (yy >> d) & 1;
        if (x) {
            if (y) res += f[node(cur, 1)], cur = node(cur, 0);
            else cur = node(cur, 1);
        } else  {
            if (y) res += f[node(cur, 0)], cur = node(cur, 1);
            else cur = node(cur, 0);
        }
    }
    return res;
...
signed main() {
  ...
  while (q--) {
      ...
      if (type == 1) modify(x, 1);
      else if (type == 2) modify(x, -1);
      else {
          cin >> y;
          cout << calc(x, y) << '\n';
      }
