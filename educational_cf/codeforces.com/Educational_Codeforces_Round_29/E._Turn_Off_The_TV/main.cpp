...
signed main() {
  ...
  for (int i = 1; i <= n; ++i) {
    ...
    s.push_back(l + 1), s.push_back(r + 1);
  }
  un(s);

  for (int i = 1; i <= n; ++i) {
    int l = int(upper_bound(s.begin(), s.end(), q[i].first) - s.begin());
    int r = int(upper_bound(s.begin(), s.end(), q[i].second) - s.begin());
    pref[r + 1] -= 1, pref[l] += 1;
  }
