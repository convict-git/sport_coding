...
inline int to_int (const string &s) {
  int x = 0;
  fr (i, 0, s.size()-1) x = x*10 + (s[i] - '0');
  return x;
}

int solve (string &s, bool print = false) {
  int cnt = 0;
  fr (len, 1, min((int)s.size(), 6)) {
    for (int i = 0; i + len - 1 <= (int)s.size() - 1; ++i) {
      string sub = s.substr(i, len);
      int int_val = to_int(sub);
      if (prime_set.find(int_val) != prime_set.end()) {
        ++cnt;
        if (print) debug(sub, int_val);
      }
    }
  }
  return cnt;
...
signed main() {
   ...
   for (int p : primes) prime_set.insert(p);
