...
struct outNode {
   ...
   struct inNode {
      set <int> s;
      void initInNode (int x) { s.insert(x); }
      void mergeInNode (const inNode &l, const inNode &r) {
         for (const int &le : l.s) s.insert(le);
         for (const int &re : r.s) s.insert(re);
      }
   };
   inNode *Ti;
...
int get (int ql, int qr, int l, int r, int x, const int &D) {
   if (l > qr || r < ql) return 0;
   if (l >= ql && r <= qr) {
      return To[x].getMeRes(D);
   }
   int mid=(l+r)/2;
   return get(ql, qr, l, mid, x+x+1, D)
      + get(ql, qr, mid+1, r, x+x+2, D);
}
