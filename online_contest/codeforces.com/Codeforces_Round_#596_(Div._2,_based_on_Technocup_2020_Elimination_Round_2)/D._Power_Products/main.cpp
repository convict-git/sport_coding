...
signed main() {
   ...
   for (int i = 0; i < n; ++i) {
     for (int p : primes)  {
       if (1ll * p * p > 1ll * a[i]) break;
       int al = 0;
       while (a[i] % p == 0) a[i] /= p, ++al;

       al %= k;
       if (al != 0) L[i].push_back(make_pair (p, al));
     }
     if (a[i] != 1) L[i].push_back(make_pair(a[i], 1));
   }
   ...
   for (int i = 0; i < n; ++i) {
     vp find;
     for (pii el : L[i]) {
       find.push_back(make_pair(el.x, (k - el.y) % k));
     }
     cnt += vpmp[find];
     ++vpmp[L[i]];
   }
