...
pair <vi, int> make_black (string s) {
  int n = sz(s);
  int cnt = 0;
  vi moves;
  fr(i, 0, n-2) {
  ...
  if (ok) return make_pair(moves, cnt);
  else return make_pair(moves, -1);
}

pair <vi, int> make_white (string s) {
  int n = sz(s);
  int cnt = 0;
  vi moves;
  fr(i, 0, n-2) {
...
signed main() {
  ...
  pair <vi, int> mw = make_white(s);
  pair <vi, int> mb = make_black(s);
  if (mw.Y == -1 && mb.Y == -1)
    cout << -1 << '\n';
  else {
    if (mw.Y != -1) {
