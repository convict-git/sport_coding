...
signed main() {
   ...
   while (p < q) {
     ...
     if (abs(bal[p].X) > abs(bal[q].X)) {
       edges.push_back(make_tuple(bal[q].Y+1, bal[p].Y+1, abs(bal[q].X)));
       bal[p].X += bal[q].X;
       bal[q].X -= bal[q].X;
       --q;
     }
     else {
       edges.push_back(make_tuple(bal[q].Y+1, bal[p].Y+1, abs(bal[p].X)));
       bal[q].X += bal[p].X;
       bal[p].X -= bal[p].X;
       ++p;
     }
