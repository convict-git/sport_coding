...
string takemid(string t) {
  string res = "";
  string trev(t);
  reverse(trev.begin(), trev.end());
  t = t + '#' + trev;
  vector <int> pi = pifunc(t);
  int takenlen = pi[(int)t.size() - 1];
  return t.substr(0, takenlen);
}
...
signed main() {
  ...
  while (tc--) {
    ...
    int idx = 0;
    string takeaway = "";
    while (idx < n/2 && s[idx] == s[n - 1 - (idx + 1) + 1]) {
      takeaway += s[idx];
      ++idx;
    }
    string res = "";
    if (idx == n/2) {
      ...
      string frnt = takemid(t);
      reverse(t.begin(), t.end());
      string bck = takemid(t);

      res += takeaway;
      if (frnt.size() >= bck.size()) res += frnt;
      else res += bck;
      reverse(takeaway.begin(), takeaway.end());
      res += takeaway;
    }
    cout << res << '\n';
  }
