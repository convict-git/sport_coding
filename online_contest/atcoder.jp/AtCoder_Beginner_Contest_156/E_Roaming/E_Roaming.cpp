...
signed main() {
  ...
  if (k <= n - 1) {
    for (int m = k; m <= n - 1; ++m) {
      int left = n - m - 1;
      res = sub(res, mul(C(n, left), left));
      // when we have n-1 moves we can have 0 0 ... 0 n 0 .. 0
      // when we have n-2 ...               1 0 .... n-1 0...0
      // when we have n-3 moves we have .. 1 1 0 0.. n-2 . 0 ..0
      // when we have n-4 moves we have ...1 1 1 0.. n-3. .0 ..0
    }
  }
  cout << res << '\n';
  return EXIT_SUCCESS;
