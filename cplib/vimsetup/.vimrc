...
set background=dark "/both work together
"
...
"
autocmd filetype cpp,java,python,haskell nnoremap <C-x> :lcd %:p:h <bar> !clear && clear && cp-tester % 3 C
...
autocmd filetype haskell nnoremap <C-c> :!clear && clear && ghci %
...
autocmd filetype cpp nnoremap <F5> :lcd %:p:h <bar> !clear && clear && cp-compile %
...
if has("autocmd")
   augroup templates_"
      ...
      autocmd BufEnter * silent! lcd %:p:h
      autocmd BufEnter .vimrc,*.vim,*.cpp,*.c,*.java,*.py,*.sml,*.hs :TagbarOpen
...
function! MrconvictAddress()
   exec '0r ~/Dropbox/myfiles/sport_coding/cplib/templates/mrconvict.cpp'
   exec 'w'
   exec '!clear && clear && yank_to_submit % 3'
   let @+=expand("%:p")
   echo expand("%:p")
