...
inline ll d (ll ds, int w) {
  if (w - ds >= 0) return w - ds;
  else return 0;
}
...
signed main() {
   ...
   while (!q.empty()) {
     ...
     for (auto vpair : G[u]) {
       ...
       if (!vis[v] && dist[v] > d(dist[u], w) + dist[u]) {
         dist[v] = d(dist[u], w) + dist[u];
         // if (!in_queue[v]) q.push({dist[v], v});
         q.push({dist[v], v});
       }
     }
   }
   if (dist[n-1] == inf) cout << "NO PATH EXISTS\n";
   else cout << dist[n-1] << '\n';
