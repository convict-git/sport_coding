...
int solve (int i, int j) {
  ...
  int &el = memo[i][j];
  el = max(solve(i - 1, j), solve(i, j - 1));
  if (a[i] == b[j]) el = max(el, solve(i - 1, j - 1) + 1);
  return el;
}
*/
signed main() {
   ...
   // fr(i, 0, len) fr(j, 0, len) memo[i][j] = -1;
   // cout << solve(len-1, len-1) << endl;
   //
   // dp[i][j] -> dp[i-1][j] , dp[i][j-1] , (s[i] == s[j] ? dp[i-1][j-1] + 1
   //
