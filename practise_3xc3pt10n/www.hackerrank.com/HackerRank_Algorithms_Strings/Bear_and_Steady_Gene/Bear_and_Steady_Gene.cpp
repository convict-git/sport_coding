...
signed main() {
   ...
   map <char, int> idx = {{'G', 0}, {'T', 1}, {'A', 2}, {'C', 3}};
   vi count(4);
   vvi pref(n + 1, vi (4, 0));

   for (int i = 1; i <= n; ++i) {
     for (int j = 0; j < 4; ++j) pref[i][j] = pref[i-1][j];
     pref[i][idx[s[i]]] = pref[i-1][idx[s[i]]] + 1;
     ++count[idx[s[i]]];
   }
   ...
   auto check = [&] (int len) -> bool {
     ...
     for (int i = 1; i + len - 1 <= n; ++i) {
       ...
       for (int j = 0; ok && j < 4; ++j) {
         left[j] = count[j] - (pref[r][j] - pref[l-1][j]);
         wanted[j] = req - left[j];
         if (wanted[j] < 0) ok = false;
         needed += wanted[j];
       }
       if (!ok) continue;
       if (needed == len) {
   ...
   int l = 0, h = n;
   while (l <= h) { // answer in on l
     // debug("Binary_Search", l, h);
     int g = (l+h)/2;
     if (check(g)) h = g - 1;
     else l = g + 1;
   }
