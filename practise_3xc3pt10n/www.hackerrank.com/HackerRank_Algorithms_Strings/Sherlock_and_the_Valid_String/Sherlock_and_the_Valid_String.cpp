...
signed main() {
   ...
   vi real_fq;
   for (int i = 0; i < 26; ++i) if (fq[i]) real_fq.push_back(fq[i]);
   for (int el : real_fq) st.insert(el);
   sort (real_fq.begin(), real_fq.end());
   ...
   if ((int)st.size() == 1) {
     ...
     bool ok = true;
     int sz = (int)real_fq.size();
     for (int i = 0; i < sz - 1; ++i)
       if (real_fq[i] + 1 != real_fq[sz - 1]) {
         ok = false;
         break;
       }
