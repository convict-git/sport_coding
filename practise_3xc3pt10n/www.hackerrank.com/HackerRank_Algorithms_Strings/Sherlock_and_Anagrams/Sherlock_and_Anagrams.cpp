...
signed main() {
   ...
   while (q--) {
     ...
     ll cnt = 0;
     string s1, s2;
     for (int l = 1; l <= len; ++l) { // 100
       for (int i = 0; i + l - 1 <= len - 1; ++i) { // 100
         s1 = s.substr(i, l); sort(s1.begin(), s1.end());
         for (int j = i + 1; j + l - 1 <= len - 1; ++j) { // 100
           s2 = s.substr(j, l); sort(s2.begin(), s2.end());
           if (s1 == s2) ++cnt;
         }
       }
     }
