...
signed main() {
   ...
   for (int i = (n - 1)/2 - 1; i >= 0; --i) {
     dp[i] = dp[i + 1] + (s[i] != s[n - i - 1]);
   }

   bool ok = true;

   for (int i = 0; i <= (n-1)/2; ++i) {
     // debug(i, s[i], n-i-1, s[n-i-1], dp[i], k);
     if (i != n - i - 1 && s[i] != s[n - i - 1]) {
       if (k >= (s[i] != '9') + (s[n - i - 1] != '9') + dp[i] - 1) { // can I set to max ?
         k -= ((s[i] != '9') + (s[n - i - 1] != '9'));
         s[i] = s[n - i - 1] = '9';
       }
       else if (k >= dp[i]) {
     ...
     else if (i != n - i - 1 && s[n-i-1] == s[i]) {
       if (k >= (s[i] != '9') + (s[n - i - 1] != '9') + dp[i]) { // can I set to max ?
         k -= ((s[i] != '9') + (s[n - i - 1] != '9'));
         s[i] = s[n - i - 1] = '9';
       }
       else if (k >= dp[i]) {
