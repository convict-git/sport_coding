...
signed main()
{
   ...
   fr(i, 0, 9) fr(j, 0, 9) fr(k, 0, 9) {
      int p = i + j + k;
      if (primes.find(p) != primes.end()) {
         th.insert(make_pair(p, k));
         fr(l, 0, 9) {
            int pf = p + l;
            if (primes.find(pf) != primes.end())  {
               fo.insert(make_pair(pf, l));
               fr (m, 0, 9) {
                  int pv = pf + m;
                  if (primes.find(pv) != primes.end())
                  {
                     fi.insert(make_pair(pv, m));
                  }
               }
            }
         }
   ...
   */
   ...
   // for (auto el : fi) cerr << el.X << ' ' << el.Y << endl;

   int dp[N][2][45], got[45], way[N];
   ...
   fr(i, 0, 9) fr(j, 0, 9) fr(k, 0, 9)
   {
      if (primes.find(i + j + k) != primes.end())
         got[i + j + k] += 1;
   }
