...
signed main() {
  ...
  cin >> n;
  for (int i = 0; i < n; ++i) cin >> ar[i], dp[i][0] = ar[i];
  lg[1] = 0;
  for (int i = 2; i < N; ++i)
    lg[i] = lg[i/2] + 1;
  ...
  for (int k = 1; k < LOG; ++k) {
    for (int i = 0; i < n; ++i) {
      int nxt = i + (1 << (k - 1));
      if (i + (1 << k) - 1 <= n - 1) dp[i][k] = max(dp[i][k  - 1], dp[nxt][k - 1]);
    }
  }
  ...
  for (int i = 0; i < n; ++i) {
    ...
    for (int k = LOG - 1; k >= 0; --k) {
      if (idx + (1 << k) - 1 <= n - 1 && dp[idx][k] <= ar[i]) {
        idx += (1 << k);
      }
    }
    lt[i] = idx == n ? 0 : idx + 1;
    ...
    idx = i;
    for (int k = LOG - 1; k >= 0; --k) {
      if (idx - (1 << k) + 1 >= 0 && dp[idx - (1 << k) + 1][k] <= ar[i]) {
        idx -= (1 << k);
      }
    }
    rt[i] = idx == -1 ? 0 : idx + 1;
    res = max(res, 1ll * lt[i] * rt[i]);
  }
