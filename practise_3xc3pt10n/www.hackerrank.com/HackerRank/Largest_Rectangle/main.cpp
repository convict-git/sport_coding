...
const int N = (int)1e5 + 10, LOG = 20;
int dp[N][LOG];
int h[N], lg[N], lt[N], rt[N];
int n;
...
signed main() {
  ...
  cin >> n;
  for (int i = 0; i < n; ++i) cin >> h[i], dp[i][0] = h[i];
  lg[1] = 0;
  for (int i = 2; i < N; ++i)
    lg[i] = lg[i/2] + 1;
  ...
  for (int k = 1; k < LOG; ++k) {
    for (int i = 0; i < n; ++i) {
      int nxt = i + (1 << (k - 1));
      if (i + (1 << k) - 1 <= n - 1) dp[i][k] = min(dp[i][k  - 1], dp[nxt][k - 1]);
    }
  }
  ...
  for (int i = 0; i < n; ++i) {
    ...
    idx = i;
    for (int k = LOG - 1; k >= 0; --k) {
      if (idx - (1 << k) + 1 >= 0 && dp[idx - (1 << k) + 1][k] >= h[i]) {
        idx -= (1 << k);
      }
    }
    rt[i] = ++idx;

    res = max(res, 1ll * h[i] * (lt[i] - rt[i] + 1));
  }
