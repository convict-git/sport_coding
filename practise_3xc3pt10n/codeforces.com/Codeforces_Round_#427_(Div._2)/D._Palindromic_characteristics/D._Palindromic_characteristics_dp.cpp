...
int n;
string s;
int dp[N][N]; // max k for substring s[i...j]
...
void solve() {
   ...
   for (int len = 2; len <= n; ++len) {
      for (int i = 0; i + len - 1 <= n - 1; ++i) {
         int j = i + len - 1;
         int mid_len = len/2;
         if (for_h(i, len, n) == rev_h(n - i - len, len, n)) {
            dp[i][j] = 1;
            if (for_h(i, mid_len, n) == for_h(i + len - mid_len, mid_len, n)) {
               dp[i][j] = max(dp[i][j], dp[i][i + mid_len - 1] + 1);
            }
         }
      }
   }
