...
inline double ABS (double x) { return x > 0 ? x : -1ll*x; }
...
bool slowLiesInside (pt qry, int layIdx) {
   CH &hull = layers[layIdx];
   pt A = hull.chVec.front(); // pivot // v[0]
   double realHullArea = 0.0; // 2 twice the actual area
   // A, vi, vi+1
   int sz = (int)hull.chVec.size();
   for (int i = 1; i < sz-1; ++i) {
      realHullArea += fabs(A.cross(hull.chVec[i], hull.chVec[i+1]));
   }
   double pointHullArea = 0.0;
   for (int i = 0; i < sz; ++i) {
      pointHullArea += fabs(qry.cross(hull.chVec[i], hull.chVec[(i+1) % sz]));
   }
   return isZero(realHullArea - pointHullArea);
}
