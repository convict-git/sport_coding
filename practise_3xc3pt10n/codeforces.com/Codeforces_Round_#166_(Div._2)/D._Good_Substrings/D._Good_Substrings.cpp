...
int gp_int (int a, int k) {
   int a_sq = int(1ll * a * a % polyhash::Mod);
   if (k == 1) return 1;
   else if (k & 1) return int((1 + (1ll * a * gp_int(a, k - 1) % polyhash::Mod)));
   else return int(1ll * (1 + a) * gp_int (a_sq, k/2) % polyhash::Mod);
}

ull gp_ull (ull a, int k) {
   ull a_sq = a * a;
   if (k == 1) return 1;
...
pair <int, ull> inf_ext(pair <int, ull>& h, int len, int o_len) {
      int p = polyhash::p;
      int hx = int(1ll * h.x * polyhash::pow_int[len] % polyhash::Mod);
      hx = int(1ll * hx * gp_int(p, o_len) % polyhash::Mod);
      ull hy = h.y * polyhash::pow_ull[len] * gp_ull(p, o_len);
...
void solve() {
   ...
   vector <pair <int, ull> > good_hashes;
   int cnt = 0;
   for (int len = 1; len <= n; ++len) {
      good_hashes.clear();
      for (int i = 0; i + len - 1 < n; ++i) {
         int j = i + len - 1;
         int query = cumm[j] - (i > 0 ? cumm[i - 1] : 0);
         if (query <= k) {
            good_hashes.push_back(hash(i, len, n));
         }
      }
      un(good_hashes);
      cnt += (int)good_hashes.size();
   }
   cout << cnt << '\n';
}
...
signed main() {
   ...
   for (int i = 0; i < n; ++i) {
      int idx = s[i] - 'a';
      cumm[i] = (good_string[idx] == '0') + (i > 0 ? cumm[i - 1] : 0);
   }
   solve();
