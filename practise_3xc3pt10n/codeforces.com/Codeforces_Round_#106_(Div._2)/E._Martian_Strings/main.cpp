...
int solve() {
  int len = (int)wo.size();
  if (len == 1) return 0;
  vector <int> pref = pref_func(wo + "#" + s);
  reverse(wo.begin(), wo.end());
  reverse(s.begin(), s.end());
  vector <int> suff = pref_func(wo + "#" + s);
  reverse(s.begin(), s.end());

  for (int i = 1; i < n; ++i)
    pref[len + 1 + i] = max(pref[len + 1 + i - 1], pref[len + 1 + i]);

  for (int i = n - 2; i >= 0; --i)
    suff[len + 1 + (n - i - 1)] = max(suff[len + 1 + (n - i - 1)], suff[len + 1 + (n - i -1) - 1]);

  for (int i = 0; i < n - 1; ++i) {
    if (pref[len + 1 + i] + suff[len + 1 + n - (i + 1) - 1] >= len)
      return 1;
  }
  return 0;
}
