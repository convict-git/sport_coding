...
int f[N], h[N], dp[N], t[N], vis[N];
vector <vector <int>> g;

inline int pos(int x) { return (x >= 0 ? x : 0); }
...
void calc(int u, int pr)
{
  ...
  if (pr != -1) {
    h[u] = t[pr] + pos(f[pr]) - pos(f[u] + t[u]) + pos(h[pr]);
  }
  dp[u] = t[u] + pos(f[u]) + pos(h[u]);
...
signed main()
{
  ...
  int n;
  cin >> n;
  for (int i = 0; i < n; ++i) cin >> t[i], t[i] = (t[i] ? 1 : -1);
