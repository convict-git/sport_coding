...
//          wgt[j][i] = wgt[i][j] = min(wgt[i][j], wgt[i][k] + wgt[k][j]);
   ...
   for (int mask = 0; mask < (1 << n); ++mask)
      for (int i = 0; i < n; ++i)
         dp[i][mask] = inf, par[i][mask] = -1, start[i][mask] = -1;
   ...
   for (int i = 0; i < n; ++i) {
      ...
      // debug(i, mask, dp[i][mask], wgt[i][start[i][mask]]);
      if (mn_cycle > go) {
         mn_cycle = go;
         pos = i;
      }
...
void solve() {
   cin >> nx >> ny >> sx >> sy >> n;
   vector <pii> point = {pii(sx, sy)};

   for (int i = 0; i < n; ++i) {
      int xx, yy;
      cin >> xx >> yy;
      point.push_back(pii(xx, yy));
   }
   ...
   for (int i = 0; i < n; ++i) for (int j = 0; j < n; ++j)
         wgt[i][j] = abs(point[i].x - point[j].x) + abs(point[i].y - point[j].y);

   ll mn_cycle = tsp();
   cout << "The shortest path has length " << mn_cycle << "\n";
}
