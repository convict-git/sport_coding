...
double givefloat (int low, int high) {
  int num = giveRand(low, high);
  int den = giveRand(1, high);
...
signed main() {
  ...
  while (tc--) {
    ...
    cout << n << '\n';
    for (int i = 0; i < n; ++i) {
      cout << givefloat(-100, 100) << ' ' << givefloat(-100, 100) << ' ' << givefloat(-100, 100) << ' ' << givefloat(-100, 100) << '\n';
    }
  }
  cout << 0 << '\n';
  return EXIT_SUCCESS;
