...
struct suffix_array {
  ...
  int longestcp (int i, int j) { // in cyclic string (NOTE, otherwise change starting value of k)
    int res = 0;
    for (int k = Log; k >= 0; --k) {
  ...
  suffix_array (string s_) {
    ...
    n = (int)s.size();
    cyclic_sort();
    for (int i = 1; i < n; ++i) { // first string will be starting from $
      sa[i - 1] = p[Log][i];
      lcp[i - 1] = longestcp(p[Log][i], p[Log][i + 1]);
    }
  }
...
signed main() {
  ...
  while (tc--) {
    ...
    for (int i = 0; i < n - 1; ++i) {
      ...
      int l = i;
      for (int k = Log; k >= 0; --k) {
        if (l - (1 << k) + 1 >= 0 && sp[k][l - (1 << k) + 1] >= h)
          l -= (1 << k);
      }
      ++l;
      int r = i;
      for (int k = Log; k >= 0; --k) {
        if (r + (1 << k) - 1 <= n - 2 && sp[k][r] >= h)
          r += (1 << k);
      }
      --r;
      chk.insert(make_pair(l, r));
    }
