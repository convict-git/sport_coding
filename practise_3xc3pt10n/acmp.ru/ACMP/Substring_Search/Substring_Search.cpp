...
struct polyhash {
   ...
   inline pair <int, ull> operator()
      (const int pos, const int len, const int mx_len = -1) const
      {
         ...
         // debug(hash_int, hash_ull, pos, len, mx_len);
...
// int polyhash::p = 53; // 31 for [a ... z], 53 for [a..z, A..Z]
string P, T;

void solve() {
   polyhash hash_p(P);
   polyhash hash_t(T);
   int len = (int)P.size(), n = (int)T.size();
   pair <int, ull> h = hash_p(0, len, n - 1);
   vector <int> ans;

   for (int i = 0; i + len <= n; ++i) {
      if (h == hash_t(i, len, n - 1))
         ans.push_back(i);
   }
