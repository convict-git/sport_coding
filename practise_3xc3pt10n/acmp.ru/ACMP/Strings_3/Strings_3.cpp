...
struct polyhash {
   ...
   polyhash (const string &st) : sz((int)st.size())
   {
      ...
      while ((int)pow_int.size() <= sz + 1) { // calc powers
...
void solve() {
   int len = (int)b.size();
   int mxp = max(2*len, (int)a.size());
   b += b;
   vector <pair <int, ull>> cycles;
   polyhash hash_b(b), hash_a(a);

   for (int i = 0; i < len; ++i) {
      cycles.push_back(hash_b(i, len, mxp));
   }
   ...
   for (int i = 0; i + len <= (int)a.size(); ++i) {
      pair <int, ull> match = hash_a(i, len, mxp);
      int l = 0, h = sz - 1;
      while (l <= h) {
         int g = (l + h)/2;
         if (cycles[g] == match) {
